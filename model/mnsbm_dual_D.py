...
class MNSBMDual_D:
    def __init__(self, C, D, K, L, alphas=None, phis=None, gammas=None,
        ...
        self.num_cat, self.num_cat_D = int(C.max() + 1), int(D.max() + 1)
        assert self.C.shape == self.D.shape
        self.fill_na = fill_na

        if not (self.C != -1).all() or not (self.D != -1).all():
            ...
            self.D_mask = (self.D != -1)
            self.D = jnp.where(self.D == -1, self.fill_na, self.D)
            self.missing = True
        else:
            self.C_mask, self.D_mask = None, None
            self.missing = False
        
        self.update_phi_g = utils_dual_D.update_phi_g
        self.update_phi_h = functools.partial(utils_mnsbm.update_phi_h)
        self.update_phi_h_D = functools.partial(utils_mnsbm.update_phi_h)
        self.update_gamma_h_D = functools.partial(utils_mnsbm.update_gamma_h)
        self.update_gamma_kl = functools.partial(utils_mnsbm.update_gamma_kl) # reuse original function
        self.update_delta_kl = functools.partial(utils_mnsbm.update_gamma_kl) # reuse original function
        self.loglik_q = functools.partial(utils_mnsbm.loglik_q)
        self.loglik_q_D = functools.partial(utils_mnsbm.loglik_q)
        self.sbm_log_lik = functools.partial(utils_mnsbm.sbm_log_lik)
        self.sbm_log_lik_slow = functools.partial(utils_mnsbm.sbm_log_lik_slow)
        self.sbm_log_lik_D = functools.partial(utils_mnsbm.sbm_log_lik)
        self.sbm_log_lik_D_slow = functools.partial(utils_mnsbm.sbm_log_lik_slow)
        ...
        if phis is None:
            if self.rand_init == 'uniform':
                ...
                self.phi_h_D = jnp.full((self.M, self.L), 1.0 / self.L)  # q(h_j)
                ...
                self.phi_h_D = jnp.asarray(np.random.dirichlet(alpha=np.ones(self.L), size=self.M))  # Shape (M, L)
                ...
                self.phi_h_D = generate_phi_h(self.M, self.L, self.init_h, concentration=concentration*0.75)
        else:
            self.phis = phis
            self.phi_g, self.phi_h, self.phi_h_D = phis['phi_g'], phis['phi_h'], phis['phi_h_D']
        ...
        if gammas is None:
            if self.rand_init == 'uniform':
                ...
                self.gamma_h_D = jnp.ones(self.L)    # q(pi^h)
                ...
                self.gamma_h_D = np.random.gamma(shape=2.0, scale=1.0, size=self.L)  # Shape (L,)
            ...
            self.gammas = gammas
            self.gamma_g, self.gamma_h, self.gamma_kl, self.delta_kl = (gammas[key] for key in ['gamma_g', 'gamma_h', 'gamma_h_D', 'gamma_kl', 'delta_kl'])
    ...
    def update_gammas_warm(self, update_ind=False):
        ...
        self.delta_kl = self.update_delta_kl(self.alpha_pi, self.phi_g, self.phi_h_D, self.D, self.K, self.L, self.D_mask, self.D_1h, self.num_cat_D, update_factor=1, update_ind=update_ind, missing=self.missing)
        ...
        self.gamma_h_D = self.update_gamma_h_D(self.alpha_h, self.phi_h_D, update_factor=1)

    def predict_phi_g(self, C_new, D_new):
        assert C_new.shape[1] == self.M and D_new.shape[1] == self.M 
        C_new_mask, D_new_mask = (C_new != -1), (D_new != -1)
        if not C_new_mask.all() or not D_new_mask.all():
            print("Detecting missing values - will use missingness mask")
            missing = True
        else:
            C_new_mask, D_new_mask = None, None
            missing = False

        if self.fitted:
            phi_g, phi_h, phi_h_D, gamma_g,gamma_kl, delta_kl = (
                self.fitted_params[keys] for keys in ('phi_g', 'phi_h', 'phi_h_D', 'gamma_g', 'gamma_kl', 'delta_kl')
            )
        else:
            phi_g, phi_h, phi_h_D = self.phi_g, self.phi_h, self.phi_h_D
            gamma_g, gamma_kl, delta_kl = self.gamma_g, self.gamma_kl, self.delta_kl
        ...
        phi_g = normalize_log_probs(self.update_phi_g(phi_h, phi_h_D, gamma_kl_di, gamma_kl_sum_di, delta_kl_di, delta_kl_sum_di, gamma_g_di, C_new, C_new_mask, D_new, D_new_mask, missing))
    ...
    def batch_vi(self, num_iters, tol=1e-6, batch_print=50, fitted=False, update_ind=True):
        ...
        if fitted:
            phi_g, phi_h, phi_h_D, gamma_g, gamma_h, gamma_h_D, gamma_kl, delta_kl = (
                self.fitted_params[keys] for keys in ('phi_g', 'phi_h', 'phi_h_D', 'gamma_g', 'gamma_h', 'gamma_h_D', 'gamma_kl', 'delta_kl')
            )
        else:
            phi_g, phi_h, phi_h_D = self.phi_g, self.phi_h, self.phi_h_D
            gamma_g, gamma_h, gamma_h_D, gamma_kl, delta_kl = self.gamma_g, self.gamma_h, self.gamma_h_D, self.gamma_kl, self.delta_kl
        ...
        for i in range(num_iters):
            ...
            # Local updates
            gamma_g_di, gamma_h_di, gamma_h_D_di = digamma(gamma_g), digamma(gamma_h), digamma(gamma_h_D)
            gamma_kl_sum_di = digamma(gamma_kl.sum(axis=2)[:, :, jnp.newaxis])
            gamma_kl_di = digamma(gamma_kl)
            delta_kl_sum_di = digamma(delta_kl.sum(axis=2)[:, :, jnp.newaxis])
            delta_kl_di = digamma(delta_kl)

            phi_g = normalize_log_probs(self.update_phi_g(phi_h, phi_h_D, gamma_kl_di, gamma_kl_sum_di, delta_kl_di, delta_kl_sum_di, gamma_g_di, self.C, self.C_mask, self.D, self.D_mask, self.missing))
            phi_h = normalize_log_probs(self.update_phi_h(phi_g, gamma_kl_di, gamma_kl_sum_di, gamma_h_di, self.C, self.C_mask, self.missing))
            phi_h_D = normalize_log_probs(self.update_phi_h_D(phi_g, delta_kl_di, delta_kl_sum_di, gamma_h_D_di, self.D, self.missing))

            # Global updates
            gamma_g = update_gamma_g(self.alpha_g, phi_g, update_factor=1)
            gamma_h = update_gamma_h(self.alpha_h, phi_h, update_factor=1)
            gamma_h_D = self.update_gamma_h_D(self.alpha_h, phi_h_D, update_factor=1)
            ...
            delta_kl = self.update_delta_kl(self.alpha_pi, phi_g, phi_h_D, self.D, self.K, self.L, self.D_mask, self.D_1h, self.num_cat_D, update_factor=1, update_ind=update_ind, missing=self.missing)

            elbo, ll, ll_D, KL_g, KL_h, KL_h_D, KL_kl, KL_kl_D = self.elbo(phi_g, phi_h, phi_h_D, gamma_g, gamma_h, gamma_h_D, gamma_kl, delta_kl, fitted=False, verbose=False)
            end_time = time.time()

            # Save iteration results
            self.training_history.append({
                "iteration": i, "ELBO": elbo,
                "LogLik": ll, "LogLikD": ll_D, "KL-g": KL_g, "KL-h": KL_h, "KL-h_D": KL_h_D, "KL-kl": KL_kl, "KL-kl_D": KL_kl_D
            })

            if jnp.abs(elbo - elbo_prev) < tol:
                print(f'Iteration {i}, ELBO: {elbo:,.3f}, Loglik: {ll:,.3f}, Loglik_D: {ll_D:,.3f}, KL-g: {KL_g:,.3f}, KL-h: {KL_h:,.3f}, KL_h_D: {KL_h_D:,.3f}, KL-kl: {KL_kl:,.3f}, KL-kl_D: {KL_kl_D:,.3f}')
        ...
        self.fitted = True
        self.fitted_params = {'phi_g': phi_g, 'phi_h': phi_h, 'phi_h_D': phi_h_D, 'gamma_g': gamma_g, 'gamma_h': gamma_h, 'gamma_h_D': gamma_h_D, 'gamma_kl': gamma_kl, 'delta_kl': delta_kl}
        self.set_posteriors()

        return phi_g, phi_h, phi_h_D, gamma_g, gamma_h, gamma_h_D, gamma_kl, delta_kl

    def elbo(self, phi_g=None, phi_h=None, phi_h_D=None, gamma_g=None, gamma_h=None, gamma_h_D=None, gamma_kl=None, delta_kl=None, fitted=False, verbose=False):
        if fitted:
            assert self.fitted
            phi_g, phi_h, phi_h_D, gamma_g, gamma_h, gamma_h_D, gamma_kl, delta_kl = (self.fitted_params[keys] for keys in ('phi_g', 'phi_h', 'phi_h_D', 'gamma_g', 'gamma_h', 'gamma_h_D', 'gamma_kl', 'delta_kl'))
        else:
            assert phi_g is not None and phi_h is not None and phi_h_D is not None
            assert gamma_g is not None and gamma_h is not None and gamma_h_D is not None
        ...
        ll_D = self.loglik_q_D(self.D, phi_g, phi_h_D, delta_kl, self.D_mask, self.missing)

        KL_g = KLD_gpi(phi_g, gamma_g, self.alpha_g)
        KL_h = KLD_gpi(phi_h, gamma_h, self.alpha_h)
        KL_h_D = KLD_gpi(phi_h_D, gamma_h_D, self.alpha_h)
        KL_kl = KLD_dirichlet(gamma_kl, self.alpha_pi, axis=2).sum()
        KL_kl_D = KLD_dirichlet(delta_kl, self.alpha_pi, axis=2).sum()

        elbo = ll + ll_D - KL_g - KL_h - KL_h_D - KL_kl - KL_kl_D

        if verbose:
            print(f'ELBO: {elbo:,.3f}, Loglik: {ll:,.3f}, Loglik_D: {ll_D:,.3f}, KL-g: {KL_g:,.3f}, KL-h: {KL_h:,.3f}, KL_h_D: {KL_h_D:,.3f}, KL-kl: {KL_kl:,.3f}, KL-kl_D: {KL_kl_D:,.3f}')

        return elbo, ll, ll_D, KL_g, KL_h, KL_h_D, KL_kl, KL_kl_D

    def loglik_fitted(self, slow=False):
        phi_g, phi_h, phi_h_D, gamma_kl, delta_kl = (self.fitted_params[keys] for keys in ('phi_g', 'phi_h', 'phi_h_D', 'gamma_kl', 'delta_kl'))
        ...
        post_h_D = jnp.argmax(phi_h_D, axis=1)

        if slow:
            return (
                self.sbm_log_lik_slow(self.C, log_dir_mean, post_g, post_h, self.num_cat, C_mask=self.C_mask),
                self.sbm_log_lik_slow(self.D, log_dir_mean_D, post_g, post_h_D, self.num_cat_D, C_mask=self.D_mask)
            )
        else:
            return (
                self.sbm_log_lik(self.C, log_dir_mean, post_g, post_h, C_mask=self.C_mask),
                self.sbm_log_lik(self.D, log_dir_mean_D, post_g, post_h_D, C_mask=self.D_mask)
            )
    ...
    def ICL(self, slow=False, verbose=False):
        ...
        g_labels, h_labels, h_D_labels = (self.posterior_dist[keys] for keys in ('g_labels', 'h_labels', 'h_D_labels'))
        K_unique = len(jnp.unique(g_labels))
        L_unique = len(jnp.unique(h_labels))
        L_D_unique = len(jnp.unique(h_D_labels))

        ICL_pen = ICL_penalty(self.N, self.M, K_unique, L_unique, self.num_cat)
        ICL_pen_D = ICL_penalty(self.N, self.M, K_unique, L_D_unique, self.num_cat_D)
        ll, ll_D = self.loglik_fitted(slow=slow)

        ICL = ll + ll_D - ICL_pen - ICL_pen_D

        if verbose:
            print(f'ICL: {ICL:,.3f}, Loglik: {ll:,.3f}, Loglik_D: {ll_D:,.3f}, ICL-penalty: {ICL_pen:,.3f}, ICL-penalty_D: {ICL_pen_D:,.3f}, K-eff: {K_unique}, L-eff: {L_unique}, L_D-eff: {L_D_unique}')

        self.ICL_fitted = {'ICL': ICL, 'Loglik': ll, 'Loglik_D': ll_D, 'ICL_pen': ICL_pen, 'ICL_pen_D': ICL_pen_D, 'K-eff': K_unique, 'L-eff': L_unique, 'L_D-eff': L_D_unique}
    ...
    def set_posteriors(self):
        ...
        self.posterior_dist = {
            'g_labels': jnp.argmax(phi_g, 1), 'h_labels': jnp.argmax(phi_h, 1), 'h_D_labels': jnp.argmax(phi_h_D, 1),
    ...
    def summary(self):
        ...
        clusters_h_D = np.unique(self.posterior_dist['h_D_labels'])
        print(f"{len(clusters_g)} row clusters:", clusters_g)
        print(f"{len(clusters_h)} col clusters:", clusters_h)
        print(f"{len(clusters_h_D)} col_D clusters:", clusters_h_D)
        ...
        for threshold in [0.5, 0.75, 0.9, 0.95, 1]:
            ...
            print(f"Cols_D with <{threshold} probability: {int((self.fitted_params['phi_h_D'].max(1) < threshold).sum())}")
    ...
    def plt_blocks(self, plt_init=False, print_labels=False):
        ...
        if plt_init:
            ...
            h_D_labels = self.phi_h_D.argmax(1)
        ...
        h_D_labels = self.fitted_params['phi_h_D'].argmax(1)
        ...
        plt_blocks(self.D, g_labels, h_D_labels, cluster_means, title=' (VI)', print_labels=print_labels)
    ...
    def fitted_model_params(self):
        ...
        params = {
            ...
            'vi_init': {
                'phi_g': self.phi_g, 'phi_h': self.phi_h, 'phi_h_D': self.phi_h_D,
                'gamma_g': self.gamma_g, 'gamma_h': self.gamma_h, 'gamma_h_d': self.gamma_h_D,
                'gamma_kl': self.gamma_kl, 'delta_kl': self.delta_kl
                },
    ...
    def export_outputs_csv(self, folderpath, model_name=None):
        ...
        post_h_D = pd.DataFrame(self.fitted_params['phi_h_D']).to_csv(os.path.join(foldersave, 'sbm_fitted_phi_h_D.csv'), index=False)
        ...
        h_D_labels = pd.DataFrame(self.posterior_dist['h_D_labels']).to_csv(os.path.join(foldersave, 'sbm_fitted_h_D_labels.csv'), index=False)
    ...
    def load_jax_model(self, filepath):
        ...
        self.phi_g, self.phi_h, self.phi_h_D, self.gamma_g, self.gamma_h, self.gamma_kl, self.delta_kl = (params['vi_init'][key] for key in ['phi_g', 'phi_h', 'phi_h_D', 'gamma_g', 'gamma_h', 'gamma_h_D', 'gamma_kl', 'delta_kl'])
        self.fitted_params = params['vi_posterior']
        self.training_history = params['training_history']
        self.ICL_fitted = params['ICL_fitted']

        assert self.phi_g.shape[1] == self.K and self.phi_h.shape[1] == self.L and self.phi_h_D.shape[1] == self.L
    ...
    def load_jax_model_dict(self, params):
        ...
        self.alpha_g, self.alpha_h, self.alpha_pi = (params['prior'][key] for key in ['alpha_g', 'alpha_h', 'alpha_pi'])
        self.phi_g, self.phi_h, self.phi_h_D, self.gamma_g, self.gamma_h, self.gamma_h_D, self.gamma_kl, self.delta_kl = (params['vi_init'][key] for key in ['phi_g', 'phi_h', 'phi_h_D', 'gamma_g', 'gamma_h', 'gamma_h_D', 'gamma_kl', 'delta_kl'])
        self.fitted_params = params['vi_posterior']
        self.training_history = params['training_history']
        self.ICL_fitted = params['ICL_fitted']

        assert self.phi_g.shape[1] == self.K and self.phi_h.shape[1] == self.L and self.phi_h_D.shape[1] == self.L
        assert self.gamma_g.shape[0] == self.K and self.gamma_h.shape[0] == self.L and self.gamma_h_D.shape[0] == self.L
        assert self.gamma_kl.shape == (self.K, self.L, self.num_cat)
